...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("👤 저자명 검색 (선택)")
    author_name = st.text_input("저자명 (해외 검색 시 영어 입력, ex: Kim J)", value="")
    if author_name.strip():
        st.success(f"👨‍🏫 포함될 저자: {author_name.strip()}")
...
if search_button and k1:
    ...
    with st.spinner("검색어를 영어로 자동 번역 중입니다..."):
        try:
            ...
            st.success(f"🔤 번역된 영어 주제어: **{final_keyword_en}**")
    ...
    # 저자명을 포함하여 RISS/KISS/Scholar 검색어 재조합
    author_str = author_name.strip()
    safe_kws = [k for k in [k1, k2, k3, author_str] if k]
    safe_keyword_kr = " ".join(safe_kws)
    encoded_safe_kr = urllib.parse.quote(safe_keyword_kr)
    
    scholar_kw_kr = f"{final_keyword_kr} AND {author_str}" if author_str else final_keyword_kr
    encoded_kw_kr = urllib.parse.quote(scholar_kw_kr)
    ...
    if db_kiss:
        st.caption("🚨 **KISS 검색 안내:** 아래 박스의 검색어(저자명 포함)를 복사하여 KISS 홈페이지에 붙여넣어 주세요.")
    ...
    if db_pubmed or db_cochrane:
        with st.spinner("해외 논문을 분석하고 한글로 요약 번역 중입니다..."):
            try:
                ...
                # 💡 [핵심] PubMed / Cochrane 용 저자 태그 추가 ([Author])
                # 저자 이름은 번역기를 돌리면 안 되므로 원본(영어)을 그대로 사용합니다.
                author_query_en = f' AND {author_str}[Author]' if author_str else ""
                
                if db_cochrane:
                    c_term = f"({final_keyword_en}){author_query_en} AND \"Cochrane Database Syst Rev\"[Journal]"
                ...
                if db_pubmed:
                    ...
                    p_term = f"({final_keyword_en}){author_query_en} AND ({t_query}) NOT \"Cochrane Database Syst Rev\"[Journal]"
        ...
        elif db_pubmed or db_cochrane:
            st.warning("선택하신 조건(또는 저자)에 맞는 해외 논문을 찾지 못했습니다.")
